# 10 · Instalador de AVACOM Biblioteca

| Campo | Valor |
|---|---|
| Estado | **Implementado.** Con Inno Setup 6, en `installer/`. Este documento fue la especificación previa y se conserva porque sus decisiones siguen vigentes |
| Construcción | `powershell -ExecutionPolicy Bypass -File installer\construir.ps1` |
| Salida | `installer\dist\AVACOM-Biblioteca-0.1.0-setup.exe`, unos 80 MB. **No se versiona**: se publica como asset de un Release de GitHub con su SHA-256 |
| Fecha de la especificación | 3 de septiembre de 2026 |
| Fecha de esta cabecera | 14 de septiembre de 2026 |

Las cuatro decisiones que la sección 4 dejaba abiertas quedaron así:

| # | Pregunta | Decisión |
|---|---|---|
| 1 | Tecnología del instalador | Inno Setup 6. Copia de carpeta, nunca MSIX |
| 2 | Carpeta de trabajo durante la instalación | No. Se elige después, en la pestaña Administración de la aplicación, como estaba especificado |
| 3 | Firma Authenticode | **Sigue sin definir.** Sin firma, SmartScreen puede advertir en equipos nuevos |
| 4 | Versión visible | `0.1.0`, definida en el guion `.iss`; `ApplicationDisplayVersion` del `.csproj` dice `0.1`. Conviene igualarlas |

Además de lo especificado, la implementación añadió: comprobación de que la
aplicación no está abierta antes de publicar, comprobación de que no se cuela
ningún artefacto MSIX en el payload, permisos de modificación para los usuarios
sobre `%ProgramData%\AVACOM\`, y una variante `-ConContenidoDemo` que **no se
distribuye a colegios** porque incluye la clave privada del equipo de
desarrollo. Ver `installer/LEEME.txt`.
...
7. **Python no se distribuye ni se invoca.** `paquetes/*.py` es
   ...
   (`README.md`, sección «Requisitos»), y el instalador no
   lo cambia.
