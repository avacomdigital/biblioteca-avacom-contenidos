# 11 · Integración del LMS con AVACOM Biblioteca · especificación original

| Campo | Valor |
|---|---|
| Estado | **Histórico · parcialmente superado.** Se escribió el 1 de septiembre de 2026 para `prototype-lms-v03`. Se conserva porque su constitución y sus historias de usuario siguen siendo la referencia; sus fases 4 a 6 describen un plan que se ejecutó de otra forma |
| Léase junto con | [00-Introduccion.md](00-Introduccion.md), [01-Contrato-Biblioteca-LMS.md](01-Contrato-Biblioteca-LMS.md), [04-api-calls.md](04-api-calls.md) y [09-posibles-cambios-conexion-lms.md](09-posibles-cambios-conexion-lms.md) |
| Fecha de esta cabecera | 14 de septiembre de 2026 |

## Qué de este documento ya no describe la realidad

| Afirmación del documento | Estado a 14 de septiembre de 2026 |
|---|---|
| «Existe en la biblioteca pero NO está expuesto por la API»: preguntas, comprobar, secuencia de lección, voz | **Superado.** Los cinco existen como capacidades `medio`, `leccion`, `evaluacion`, `comprobar` y `voz`. Ver documento 04 |
| «No hay señal barata de que el catálogo cambió» | **Superado.** `huella_catalogo` en `/v1/salud` |
| D-3 propone `generacion` como contador monótono | **No adoptado.** Se publica `huella_catalogo` y el motivo está en el documento 01, sección 3.6 |
| D-2 propone `POST /v1/banco/{ref}/extraer` y `POST /v1/repaso` | **No implementados.** El LMS los espera. Ver documento 09, PC-02 |
| `CONTRATO-LMS.txt` lista ocho tipos de elemento | **Superado.** El documento 04 lista los diez del manifiesto, y `CONTRATO-LMS.txt` ya no existe |
| El LMS de referencia es `prototype-lms-v03` | **Superado.** El LMS actual es `prototype-lms-v04`, con su propia especificación, que traslada la administración de cursos a la Biblioteca. Ver documento 09 |
| Fase 6 «No iniciada» | **Superado en parte.** Las etapas E1 y E2 del lado de la Biblioteca están hechas; E3 a E8 son del lado del LMS y se siguen en su repositorio |

Lo que sigue es el documento original, sin modificar salvo las rutas
relativas, que ahora apuntan desde `specs/`.
...
[01-Contrato-Biblioteca-LMS.md](01-Contrato-Biblioteca-LMS.md) sin perder ningún campo, y campos
...
**T-36** `[A]` Actualizar [01-Contrato-Biblioteca-LMS.md](01-Contrato-Biblioteca-LMS.md): los diez tipos
...
Requisitos, tal como los declara [README.md](../README.md):
...
[01-Contrato-Biblioteca-LMS.md](01-Contrato-Biblioteca-LMS.md) y es la forma canónica de leer la nota,
...
[README.md](../README.md): un flujo de medio tiene que quedarse con su **propia
...
- [ ] [01-Contrato-Biblioteca-LMS.md](01-Contrato-Biblioteca-LMS.md) describe lo que el código hace *(T-36)*
...
| [01-Contrato-Biblioteca-LMS.md](01-Contrato-Biblioteca-LMS.md) | Equipo de LMS. El contrato v1 completo |
| [INVENTARIO.txt](../INVENTARIO.txt) | Quien desarrolla el componente. Qué es cada archivo y por qué |
| [README.md](../README.md) | Arquitectura, decisiones tomadas, diagnóstico de fallos |
